...
import { UserInfoPropsBase } from '../../lib/utils/interface/interface';

export default function Search() {
    // console.log("Search렌더링");
    const [search, setSearch] = useState('');
    //모든 유저 목록
    const [userList, setUserList] = useState<UserInfoPropsBase[]>([]);
    //현재 사용자에게 보여지는 유저 목록
    const [showUser, setShowUser] = useState<UserInfoPropsBase[]>([]);
    //페이지 상태관리
    ...
    const delayLoading = useCallback(
        debounce((q) => setIsLoading(false), 500),
        []
    );

    const handleTyping = (e: INPUT_EVENT) => {
